...
def tts_to_pcm_bytes(client: OpenAI, text: str, model: str, voice: str) -> bytes:
    if model.startswith("gemini-"):
        from shared.audio.gemini import synthesize

        return synthesize(text, model=model, voice=voice)
...
async def _prepare_input(
    *,
    ...
    if config.offline:
        ...
        caller_voice = os.getenv("GEMINI_TTS_VOICE", "Kore") if config.tts_model.startswith("gemini-") else config.voice
        if audio_cache is None:
            input_pcm = await asyncio.to_thread(tts_to_pcm_bytes, tts_client, user_text, config.tts_model, caller_voice)
        else:
            cached = await asyncio.to_thread(
                ...
                synthesize=lambda: tts_to_pcm_bytes(tts_client, user_text, config.tts_model, caller_voice),
            )
