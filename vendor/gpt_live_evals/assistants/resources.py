...
class AssistantResources:
    ...
    def create_executor(
        ...
        *,
        ...
        if os.getenv("EVAL_DOMAIN") == "retail":
            from voice_eval.retail.data import task
            from voice_eval.retail.executor import RetailExecutor
            tid = initial_state.get("task_id", "21")
            return RetailExecutor(initial_state=initial_state, task=task(tid) if tid == "control" else None)
        if remote:
...
def assistant_resources(*, assistant_mode: str = "responses") -> AssistantResources:
    ...
    return AssistantResources(
        ...
        system_prompt_file=Path(
            os.getenv("EVAL_FRONTEND_PROMPT") or ASSISTANTS_DIR / "frontend" / "prompts" / "voice.txt"
        ),
        backend_system_prompt_file=Path(os.getenv("EVAL_BACKEND_PROMPT") or backend / "prompts" / "backend.txt"),
        tools_file=Path(os.getenv("EVAL_TOOLS_FILE") or backend / "tools" / "definitions.json"),
        facts_file=Path(os.getenv("EVAL_RESTAURANT_FACTS") or backend / "tools" / "restaurant_facts.json"),
    )
